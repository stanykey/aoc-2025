...
The Elves are trying to figure out **which junction boxes to connect** so that electricity can reach **every** junction
box. They even have a list of all of the junction boxes' positions in 3D space (your puzzle input).
...
together as possible** according to [straight-line distance](https://en.wikipedia.org/wiki/Euclidean_distance). In this
...
**circuit**. After connecting them, there is a single circuit which contains two junction boxes, and the remaining 18
...
single circuit which contains **three** junction boxes and an additional `17` circuits which contain one junction box
each.
...
circuit which contains **4** junction boxes, two circuits which contain **2** junction boxes each, and seven circuits
which each contain a single junction box. Multiplying together the sizes of the three largest circuits (`5`, `4`, and
one of the circuits of size `2`) produces **40**.
